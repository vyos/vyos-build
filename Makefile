...
# OCI container image as generated by `make oci` - newest artifact wins. Lazily
# evaluated so `make oci test-oci` picks up the tarball created by the former.
# If it stays empty check-oci-container does the very same lookup and reports a
# proper error message.
OCI_PATH = $(shell ls -t vyos-*-oci-$(ARCH).tar.xz 2>/dev/null | head -n1)
...
.ONESHELL:
test-oci:
	scripts/check-oci-container --debug $(if $(OCI_PATH),--image $(OCI_PATH),) $(filter-out $@,$(MAKECMDGOALS))
